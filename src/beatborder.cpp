...
// make spBeat an intrusive smart pointer
...
void BeatBorder::animateBeatHit(Beat * beat)
{
	...
	Beat * nbeat = new Beat(beat->getRadius(),beat->getType());
...
void BeatBorder::animateBeatMiss(Beat * beat)
{
	// add a fading out tween
	spTween fade_tween = createTween(Actor::TweenAlpha(0),TWEEN_DURATION,1,false);
	Tween::EASE ease = Tween::ease_outInBounce;
	...
	// TODO: figure out how to make the animation appear without the beatborder there
	//this->addChild(nbeat);
	this->prependChild(nbeat);
}
